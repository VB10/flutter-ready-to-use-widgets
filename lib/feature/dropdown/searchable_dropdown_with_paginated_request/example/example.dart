...
class SearchAbleDropdownWithPagintedRequestExample extends StatelessWidget {
  const SearchAbleDropdownWithPagintedRequestExample({Key? key}) : super(key: key);
  ...
  Future<AnimeList?> getCustomerList({required int page, String? key}) async {
    try {
      ...
      var response = await NetworkService.instance.networkManager.send<AnimeList, AnimeList>(
        url,
